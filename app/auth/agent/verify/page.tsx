...
function VerifyContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await verifyOtp(email, code, type, "agent");
    ...
    if (result?.error) {
      ...
      if (type === "recovery") {
        router.push("/auth/agent/update-password");
      } else if (signupRole === "agent") {
        // This was a signup — ensure agentdata & roles are created
        localStorage.removeItem("signupRole");
        const ensure = await ensureAccountAfterSignup("agent");
        if (ensure?.error) {
